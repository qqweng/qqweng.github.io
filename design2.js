...
var obj = [
    ...
    "The goal of this program is to understand the differences between C and C++ programming and experience how programmers continuously adapt and change their applications to make it more effient and function better.",
...
var about = [
    ...
    "This appointment calendar program was created with my partner, Emma Guo, where we continuously changed the program to adapt it's usages to better fit the client's needs over a course of a month with the concepts of C/C++ programming<br>",
...
var moreInfo = [
    ...
    "In the beginning of the project, the students were given a BNF and grammatical structure of E programming to understand to parse and translate the code to Java. From there, we start off with creating a scanner so that we can break up the input into tokens. Next, we develop a parser to reorganize the grammar using syntax graphs. The next part is one of the hardest parts of the program which is to create the symbol table. I approached the problem by using stacks and pushing variables through each scope appropriately by following the guideline of how E programming works. The last part is generating the code so that the translator creates a equivalent Java code that works the same as its E code counterpart. <button onclick=\"myFunction()\" id=\"link2\">Read less</button>"
]
